-- Refactor scoring into a single source of truth: expose the per-match score
-- granularity that leaderboard_entries already computed and then summed away.
-- match_prediction_scores holds the unchanged scoring pipeline (copied verbatim
-- from 003); leaderboard_entries is recreated to aggregate FROM it, so its output
-- stays byte-for-byte identical while the scoring SQL is no longer duplicated.
...
select
  ...
  coalesce(sum(match_prediction_scores.correct_winner::integer), 0)::integer as winner_count
