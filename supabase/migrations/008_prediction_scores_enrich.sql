-- Enrich match_prediction_scores for the leaderboard "Pontos por jogo" tab
-- (closes #49 and #55).
--
-- The per-match detail tab on the leaderboard shows only Match / pts / Rank /
-- Change. To surface the player's predicted scoreline, the official result, and
-- which scoring criteria fired (badges), the match_prediction_scores view needs
-- to expose the prediction/result goals and the per-bonus boolean flags that are
-- already computed inside its CTEs.
--
-- This migration drops and recreates both views in dependency order
-- (leaderboard_entries depends on match_prediction_scores). The CTE body is
-- BYTE-IDENTICAL to 006_draw_scoring_fix.sql; the only change is the final
-- SELECT of match_prediction_scores, which threads through:
--   * prediction_home_goals, prediction_away_goals
--   * result_home_goals, result_away_goals
--   * winner_goals_bonus, goal_difference_bonus, loser_goals_bonus,
--     rout_bonus, extra_time_bonus, penalties_bonus (booleans mirroring the
--     bonus CASE expressions, honouring the exact-score stacking rules)
-- leaderboard_entries is recreated BYTE-IDENTICAL to 006 (its output columns do
-- not change).
...
prediction_scores as (
  select
    ...
    (
      (correct_winner or correct_draw) and resolution = 'penalties'
    ) as penalties_bonus,
    case
